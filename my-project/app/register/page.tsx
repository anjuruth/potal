...
const ROLES = [
  { value: 'student', label: 'Student', desc: 'I am a student looking for placement opportunities' },
  { value: 'faculty_advisor', label: 'Faculty Advisor', desc: 'I advise and mentor students' },
  { value: 'placement_officer', label: 'Placement Officer', desc: 'I manage placement drives and companies' },
  { value: 'higher_authority', label: 'Higher College Authority', desc: 'I oversee college-level placement activities' },
  { value: 'super_admin', label: 'Super Admin', desc: 'System administrator' },
];
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (selectedCollege) {
      generalApi.getDepartments(selectedCollege).then(r => setDepartments(r.data)).catch(() => {});
    }
  }, [selectedCollege]);
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="card p-8 shadow-lg">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 border border-slate-200 dark:border-slate-600 rounded-lg py-2.5 text-slate-700 dark:text-slate-300 font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors mb-6"
          >
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-2 bg-white dark:bg-slate-800 text-slate-400">or register with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select {...register('college_id', { required: 'College required' })} className="input" onChange={e => setSelectedCollege(e.target.value)}>
                  <option value="">Select College</option>
                  {colleges.map(c => <option key={c.college_id} value={c.college_id}>{c.college_name}</option>)}
                </select>
                {errors.college_id && <p className="text-red-500 text-xs mt-1">{errors.college_id.message as string}</p>}
              </div>

              <div>
                <label className="label">Department</label>
                <select {...register('department_id')} className="input" disabled={!selectedCollege}>
