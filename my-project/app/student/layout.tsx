...
import {
  LayoutDashboard, UserIcon, Code2, Briefcase, BookOpen, FileText,
  Bell, LogOut, Menu, Sun, Moon, Trophy, Upload, Briefcase as BIcon,
} from 'lucide-react';
...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const u = getUser();
    if (!u || u.role !== 'student') { router.push('/auth/login'); return; }
    setUser(u);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
        <div className="flex items-center gap-3">
          {user?.photo ? (
            <img src={user.photo.startsWith('http') ? user.photo : `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}${user.photo}`} alt="" className="w-9 h-9 rounded-full object-cover" />
          ) : (
            <div className="w-9 h-9 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center">
              <span className="text-blue-600 font-semibold text-sm">{user?.full_name?.charAt(0) || 'S'}</span>
            </div>
          )}
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 sm:px-6 h-14 flex items-center gap-4 shrink-0">
          <button onClick={() => setOpen(true)} className="lg:hidden p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700">
            <Menu className="w-5 h-5" />
          </button>
          <div className="flex-1">
            <h2 className="text-sm font-semibold text-slate-500 dark:text-slate-400">
              {nav.find(n => n.href === pathname)?.label || 'Student Portal'}
            </h2>
          </div>
